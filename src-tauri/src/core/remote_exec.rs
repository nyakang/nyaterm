...
pub async fn exec_ssh_session_command(
    ...
    //
    // The handle creation (which may perform a full SSH connect+auth) is
    // bounded by the same timeout as the command itself, so that a stalled
    // connection or host-key/auth prompt doesn't hang callers indefinitely.
    let ssh_handle = tokio::time::timeout(
        timeout,
        get_or_create_independent_handle(manager, session_id),
    )
    .await
    .map_err(|_| AppError::Channel("SSH connection for remote exec timed out".to_string()))??;
